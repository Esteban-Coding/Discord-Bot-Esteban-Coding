...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption((option) =>
      option.setName("usuario").setDescription("a distorsionar")
    )
    .addAttachmentOption((option) =>
      option.setName("imagen").setDescription("a distorsionar")
    )
    .addStringOption((option) =>
      option.setName("url").setDescription("a distorsionar")
    ),
  ...
  async execute(interaction, client) {
    ...
    if (usuario != null) {
      ...
      status[0] = "⌛ㅤ";
      request(
        `https://nekobot.xyz/api/imagegen?type=magik&image=${usuario.user.displayAvatarURL(
          { extension: "png", forceStatic: true, size: 512 }
    ...
    if (link != null) {
      ...
      status[2] = "⌛";
      request(
        `https://nekobot.xyz/api/imagegen?type=magik&image=${link}&intensity=${Math.ceil(
          Math.random() * 10
        )}`,
        function (error, response, body) {
          if (error || !body || !JSON.parse(body).success) {
