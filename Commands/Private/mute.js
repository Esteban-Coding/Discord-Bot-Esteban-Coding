...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("mute")
    .setDescription("🔇 Silencia a un usuario (MOD)")
    .setDefaultMemberPermissions(PermissionFlagsBits.MuteMembers)
    .addUserOption((option) =>
      option.setName("usuario").setDescription("a silenciar").setRequired(true)
    )
    .addStringOption((option) =>
      option.setName("motivo").setDescription("del silencio").setRequired(true)
    )
  ...
  execute(interaction, client) {
    ...
    if (duracion > 43800)
      return interaction.reply({
        content:
          ":warning: ***El limite de duración es de 1 mes (43.800 minutos).***",
        ephemeral: true,
      });
    if (user.id === client.user.id)
      return interaction.reply({
        content: ":no_entry: ***No puedes silenciarme.***",
        ephemeral: true,
      });
    if (user.id === interaction.user.id)
      return interaction.reply({
        content: ":no_entry: ***No puedes silenciarte a ti mismo.***",
        ephemeral: true,
      });
    if (
      ...
      return interaction.reply({
        content:
          ":no_entry: ***No puedes silenciar a otros moderadores/administradores.***",
        ephemeral: true,
      });
