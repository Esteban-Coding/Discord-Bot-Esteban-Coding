...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("ban")
    .setDescription("🚷♾️ Banea a un usuario (MOD)")
    .setDefaultMemberPermissions(PermissionFlagsBits.BanMembers)
    .addUserOption((option) =>
      option.setName("usuario").setDescription("a banear").setRequired(true)
    )
    .addStringOption((option) =>
      option.setName("motivo").setDescription("del baneo").setRequired(true)
    ),
    ...
    try {
      ...
      if (user.id === client.user.id)
        return interaction.reply({
          content: ":no_entry: ***No puedes banearme.***",
          ephemeral: true,
        });
      if (user.id === interaction.user.id)
        return interaction.reply({
          content: ":no_entry: ***No puedes banearte a ti mismo.***",
          ephemeral: true,
        });
      if (
        ...
        return interaction.reply({
          content:
            ":no_entry: ***No puedes banear a otros moderadores/administradores.***",
          ephemeral: true,
        });
