...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("clear")
    .setDescription(
      "🧹 Eliminar cierta cantidad de mensajes de un canal o de un miembro (MOD)"
    )
  ...
  async execute(interaction) {
    ...
    if (cantidad <= 0 || cantidad >= 100) {
      return interaction.reply(
        ":x: **NUMERO INVALIDO. (LIMITE DE 99 MENSAJES)**"
      );
    }
    ...
    if (miembro) {
      ...
      await channel.bulkDelete(filtered).then((messages) => {
        embed.setDescription(
          `**SE ELIMINARON CORRECTAMENTE** ${messages.size} **MENSAJES DE** ${miembro}**.**`
        );
      ...
      await channel.bulkDelete(cantidad, true).then((messages) => {
        embed.setDescription(
          `**SE ELIMINARON CORRECTAMENTE** ${messages.size} **MENSAJES DEL CANAL.**`
        );
        interaction.reply({ embeds: [embed] });
      });
    }
