...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("post")
    .setDescription("📢 Cuadro de contenido personalizado (MOD)")
    .setDefaultMemberPermissions(PermissionFlagsBits.BanMembers)
    .addStringOption((option) =>
      option.setName("titulo").setDescription("a repetir").setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("contenido")
    ...
    .addStringOption((option) =>
      option.setName("url").setDescription("dirección url en el titulo")
    )
    .addStringOption((option) =>
      option.setName("miniatura").setDescription("Imagen miniatura (URL)")
    )
    .addStringOption((option) =>
      option.setName("imagen").setDescription("Imagen del contenido (URL)")
    )
    .addAttachmentOption((option) =>
      option
        .setName("multimedia")
        .setDescription(
          "Archivos multimedia (documentos, imagenes, videos, audios)"
        )
    ),
  ...
  execute(interaction, client) {
    ...
    if (color.charAt(0) != "#") {
      return interaction.reply({
        content: ":x: ***COLOR SIN FORMATO HEX***",
        ephemeral: true,
      });
    }
    ...
    try {
      ...
      if (archivo === null) {
        ...
        client.channels.cache
          .get(interaction.channelId)
          .send({ embeds: [embed], files: [archivo] });
      }
    } catch {
